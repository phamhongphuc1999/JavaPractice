...
public class UnitController {
  ...
  public ResponseEntity<Responder> getAll() {
    try {
      ...
      return ResponseEntity.ok().body(new Responder("success", units));
    } catch(Exception exception) {
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new Responder("Bad Request", exception.getMessage()));
    }
  }
}
